...
// console.log(pelis[0].genres[0].name);
        ...
        if ( input.value != ""){
            ...
            for (let i = 0; i < pelis.length; i++) {
                pelis.forEach(peli => { //recorrer
                    // console.log(peli.genres.name)
                    showContent(pelis[i].genres[i].name.filter( filtrados => filtrados.genres[i].name.toLowerCase().includes(input.value)))
                });
